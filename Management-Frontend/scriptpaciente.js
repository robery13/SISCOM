...
function renderizarPersonalizacion() {
  ...
  container.innerHTML = `
      ...
      <div id="perfilFormBloque" class="d-none">
        <div class="perfil-datos-grid">
          ...
          <div style="grid-column: 1 / -1;">
            ...
            <input id="perfilEmail" type="email" class="form-control" placeholder="correo@dominio.com" disabled>
            <div class="form-text">El correo no se puede modificar porque es tu dato de acceso.</div>
          </div>
        </div>
    ...
    <div class="card mb-4 border-0 shadow-sm">
      <h4 class="mb-3"><i class="bi bi-key-fill"></i> Cambiar Contraseña</h4>
      <p class="text-muted mb-3">Necesitas tu contraseña actual para poder cambiarla.</p>
      <div id="cambioPasswordMiPerfilMsg"></div>
      <div class="perfil-datos-grid">
        <div>
      ...
      <button class="btn btn-primary" onclick="cambiarPasswordMiPerfil()">
...
// Cambio de contraseña voluntario, disponible en Mi Perfil > Personalización.
// Usa el mismo endpoint /cambiar-password que ya usa el resto del sistema.
async function cambiarPasswordMiPerfil() {
  const token = getAuthToken();
  const msgDiv = document.getElementById('cambioPasswordMiPerfilMsg');
  const pintarMsg = (texto, tipo) => {
    if (!msgDiv) return;
    msgDiv.innerHTML = `<div class="alert alert-${tipo === 'error' ? 'danger' : 'success'} py-2">${texto}</div>`;
  };
  ...
  const passwordActual = document.getElementById('miPerfilPasswordActual')?.value || '';
  const passwordNueva = document.getElementById('miPerfilPasswordNueva')?.value || '';
  const passwordConfirmar = document.getElementById('miPerfilPasswordConfirmar')?.value || '';

  if (!passwordActual || !passwordNueva || !passwordConfirmar) {
    pintarMsg('Completa los 3 campos de contraseña.', 'error');
    return;
  }
  if (passwordNueva !== passwordConfirmar) {
    pintarMsg('La nueva contraseña y su confirmación no coinciden.', 'error');
    return;
  }
  if (passwordNueva === passwordActual) {
    pintarMsg('La nueva contraseña no puede ser igual a la actual.', 'error');
    return;
  }

  const passwordRegex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*])[A-Za-z\d!@#$%^&*]{8,}$/;
  if (!passwordRegex.test(passwordNueva)) {
    pintarMsg('La nueva contraseña no cumple los requisitos mínimos de seguridad.', 'error');
    return;
  }
  ...
  try {
    ...
    if (!response.ok || !data.ok) {
      pintarMsg(data.mensaje || 'No se pudo cambiar la contraseña.', 'error');
      return;
    }

    pintarMsg(data.mensaje || 'Contraseña actualizada correctamente.', 'success');
    document.getElementById('miPerfilPasswordActual').value = '';
    document.getElementById('miPerfilPasswordNueva').value = '';
    document.getElementById('miPerfilPasswordConfirmar').value = '';
  } catch (error) {
    console.error('Error al cambiar contraseña:', error);
    pintarMsg('Error de conexión al cambiar la contraseña.', 'error');
  }
}
...
(function () {
  ...
  // En main_paciente.html el acceso a "Mi Perfil" ya existe (se entra dando
  // clic al avatar/tarjeta de usuario, con pestañas de Ficha/Contactos/
  // Personalización/Recompensas). Por eso aquí NO se inserta el botón
  // genérico "Perfil" flotante (quedaría duplicado y feo debajo del avatar).
  // El cambio de contraseña voluntario del paciente vive dentro de la
  // pestaña "Personalización" (ver cambiarPasswordMiPerfil más abajo).
